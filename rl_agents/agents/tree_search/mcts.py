...
class MCTS(AbstractPlanner):
    ...
    def reset(self):
        # there is no notion of an env state or observation associated with the node itself. 
        # the edges (aka the children) that can attached to this node will be all the actions available from the state that corresponds to this node
        # this happens when this node is expanded
    ...
    def run(self, state, observation):
        ...
        :param observation: the corresponding observation. only used if the self.root Node has not been expanded yet. Otherwise, will be overwritten
        as the tree is navigated using the tree policy (UCT)
        """
        node = self.root
        total_reward = 0
        depth = 0
        ...
        if not node.children \
            ...
            # this is a leaf node and the action edges need to be added. 
            node.expand(self.prior_policy(state, observation))

        if not terminal:
            # run the rollout policy from the leaf node till end. 
    ...
    def plan(self, state, observation):
        for i in range(self.config['episodes']): #number of mcts iterations from the same root node. Each iterations
            if (i+1) % 10 == 0:
                logger.debug('{} / {}'.format(i+1, self.config['episodes']))
            self.run(safe_deepcopy_env(state), observation)
            # what is saved between different self.runs is the MCTSNode called self.root which is set during the reset() call. The value function for the 
            # self.root MCTSNode will be updates between different runs
